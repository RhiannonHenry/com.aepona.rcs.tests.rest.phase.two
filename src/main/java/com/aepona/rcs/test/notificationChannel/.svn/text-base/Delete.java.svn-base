...
public class Delete {
    ...
    public void deleteUser1Subscription() {
        ...
        // applicationPassword).expect().log().ifError().statusCode(200).when().delete(url);
        Response response =
                RestAssured.with()
                           .auth()
                           .preemptive()
    ...
    private void createNotificationChannel(String userID, int i) {
        ...
        // applicationPassword).body(validLongPoll).expect().log().ifError().statusCode(201).post(url);
        Response response =
                RestAssured.given()
                           .auth()
                           .preemptive()
